...
class Component extends React.PureComponent {
  ...
  componentDidMount() {
    ...
    // Force enter animation to trigger visual elements
    if (this.enter) {
      this.enter();
    }
  ...
  draw() {
    ...
    // If dimensions are 0, retry drawing in next frame/tick
    if (width === 0 || height === 0) {
    ...
    this.setState({
      ...
      // Re-trigger animations if needed once valid state is set
      if (this.props.energy && this.props.energy.entered) {
        this.enter();
      }
    });
  }
